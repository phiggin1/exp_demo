...
#include <exp_demo/SegmentedClustersArray.h>
...
class segmentation {
	public:
		explicit segmentation(ros::NodeHandle nh) : m_nh(nh)  {
			...
			m_pub = m_nh.advertise<sensor_msgs::PointCloud2> ("/objects",1);
			m_clusterPub = m_nh.advertise<exp_demo::SegmentedClustersArray> ("object_clusters",1);
			...
			ROS_INFO("\n min_range: %f\n max_range: %f\n distance_threshold: %f\n cluster_tolerance: %f\n min_cluster_size: %d\n max_cluster_size: %d\n", 					min_range, max_range, distance_threshold, cluster_tolerance, min_cluster_size, max_cluster_size);
	  	}
	...
	private:
		...
		double max_range = 5.5; //0.5 m
		double min_range = 0.5; //1.5 m
		double distance_threshold = 0.04; //4cm
...
==============================*/
  ...
  exp_demo::SegmentedClustersArray CloudClusters;
